...
public class RollViewPage implements OnPageChangeListener {
    ...
    private List<PicBean> banList;//图片地址和View混合集合
    ...
    private int dotWidth;//小圆点尺寸
    private int dotHeight;//小圆点尺寸
    ...
    public static RollViewPage build(BannerBuilder builder,int dotShape) {
        return new RollViewPage(builder,dotShape);
    }

    private RollViewPage(BannerBuilder builder,int dotShape) {
     ...
     * 初始化小圆点背景
     */
    private void initDotBg(int dotShape) {
        selectorBg = new PointRoundDrawable(dotSelectorColor,dotShape);
        normalBg = new PointRoundDrawable(dotNormalColor,dotShape);
    }
    ...
    private void initViewpagerListener() {
        initAdapter();
        if (isCardMode) {//卡片式的
            viewPager.setPadding(padding, 0, padding, 30);
        ...
        if (isShowDot) {//控制是否显示小点
            ...
            if (count > 1) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    LinearLayout.LayoutParams lp_dot = new LinearLayout.LayoutParams(dotWidth, dotHeight);
                    lp_dot.setMargins(dotWidth, 0, 0, 0);
