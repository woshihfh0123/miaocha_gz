...
public class PhoneUtils {
    ...
    public static void sendSms(String phoneNumber, String content) {
        Uri uri = Uri.parse("smsto:" + (Utils.isEmpty(phoneNumber) ? "" : phoneNumber));
        Intent intent = new Intent(Intent.ACTION_SENDTO, uri);
        intent.putExtra("sms_body", Utils.isEmpty(content) ? "" : content);
        BaseApp.getInstance().startActivity(intent);
    }
    ...
    public static void sendSmsSilent(String phoneNumber, String content) {
        ...
        PendingIntent sentIntent = PendingIntent.getBroadcast(BaseApp.getInstance(), 0, new Intent(), 0);
    ...
    public static List<HashMap<String, String>> getAllContactInfo() {
        ...
        while (cursor.moveToNext()) {
            ...
            // 判断contact_id是否为空
            if (!Utils.isEmpty(contact_id)) {//null   ""
