...
import static com.mc.phonelive.AppConfig.CAMERA_IMAGE_PATH_PT;
...
//            mVideoEditer.setTXVideoReverseListener(null);
     ...
     * 设置视频输出路径
     */
    public String generatePicOutputPath() {
        String outputDir = AppConfig.VIDEO_PATH;
        File outputFolder = new File(outputDir);
        if (!outputFolder.exists()) {
            outputFolder.mkdirs();
        ...
        String videoName = DateFormatUtil.getVideoCurTimeString() + mRandom.nextInt(9999);
        return CAMERA_IMAGE_PATH_PT + "pic_" + AppConfig.getInstance().getUid() + "_" + videoName + ".jpg";
    }
