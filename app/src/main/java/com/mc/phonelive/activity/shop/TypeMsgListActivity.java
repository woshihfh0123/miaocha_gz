...
import com.mc.phonelive.adapter.MsgPubAdapter;
import com.mc.phonelive.bean.XhListBean;
...
public class TypeMsgListActivity extends AbsActivity {
    ...
    private void getList() {
        HttpUtil.getXhList(page, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (!DataSafeUtils.isEmpty(info)) {
                    JSONObject obj = JSON.parseObject(info[0]);
                    ArrayList<XhListBean> nowsBean = (ArrayList<XhListBean>) JSON.parseArray(obj.getString("list"), XhListBean.class);
