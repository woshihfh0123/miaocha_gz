...
public class AndroidUtils {
    ...
    public static void requestPermission(Activity context, String[] permissions) throws Exception {
        if (Build.VERSION.SDK_INT >= 23) {
            Class<?> activityClass = Class.forName("android.app.Activity");
            Method requestPermissionsMethod = activityClass.getMethod("requestPermissions", String[].class, int.class);
            requestPermissionsMethod.invoke(context, permissions, 0);
        } else {
            Toast.makeText(context, "缺少必要权限，请检查Manifest文件，并在设置中打开对应权限", Toast.LENGTH_LONG).show();
        }
    }
