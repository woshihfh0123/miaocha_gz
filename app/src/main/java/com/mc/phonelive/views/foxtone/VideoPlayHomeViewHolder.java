 ...
 * 视频播放器--主要是针对首页，因为生命周期没用，只能新建一个，单独作为首页的播放器使用，主要目的 是实现eventBus事件传输功能，免得和VideoPlayActivity类冲突
 * 功能主要是实现倒计时，免得两个倒计时一起了
 */

public class VideoPlayHomeViewHolder extends AbsViewHolder implements ITXVodPlayListener, View.OnClickListener {

    private TXCloudVideoView mTXCloudVideoView;
    private View mVideoCover;
    private TXVodPlayer mPlayer;
    public boolean mPaused;//生命周期暂停
    public boolean mClickPaused;//点击暂停
    ...
    public VideoPlayHomeViewHolder(Context context, ViewGroup parentView) {
    ...
    public void pausePlay() {
        ...
        if (!mClickPaused && mPlayer != null) {
            mPlayer.pause();
            EventBus.getDefault().post(new EventBusModel("pause_video_videoplay"));//这个只针对VideoPlayActivity这个类的倒计时
        }
    }
    ...
    public void clickTogglePlay() {
        ...
        if (mClickPaused) {
            ...
            EventBus.getDefault().post(new EventBusModel("main_video_puase1"));
        } else {
            hidePlayBtn();
            EventBus.getDefault().post(new EventBusModel("main_video_play1"));
        }
    }
