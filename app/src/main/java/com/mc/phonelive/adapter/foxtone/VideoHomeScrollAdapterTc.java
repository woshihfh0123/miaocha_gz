...
import com.mc.phonelive.custom.ItemSlideHelper;
import com.mc.phonelive.utils.ClickUtil;
import com.mc.phonelive.utils.DataSafeUtils;
import com.mc.phonelive.utils.IconUtil;
import com.mc.phonelive.views.foxtone.VideoPlayHomeWrapViewHolder;
...
public class VideoHomeScrollAdapterTc extends RecyclerView.Adapter<VideoHomeScrollAdapterTc.Vh> implements ItemSlideHelper.Callback {

    private static final String TAG = "VideoScrollAdapter";
    private static final int COUNT = 20;//接口每页返回多少条
    private Context mContext;
    private List<VideoBean> mList;
    private SparseArray<VideoPlayHomeWrapViewHolder> mMap;
    ...
    public VideoHomeScrollAdapterTc(Context context, List<VideoBean> list, int curPosition) {
        mContext = context;
        mList = list;
    ...
    public Vh onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        VideoPlayHomeWrapViewHolder vpvh = new VideoPlayHomeWrapViewHolder(mContext, parent);
    ...
    public void onBindViewHolder(@NonNull Vh vh, int position, @NonNull List<Object> payloads) {
        Log.v("tags",payloads.size()+"-----size");
        Object payload = payloads.size() > 0 ? payloads.get(0) : null;
        vh.setData(mList.get(position), position, payload);
        if (mFirstLoad) {
            mFirstLoad = false;
            VideoPlayHomeWrapViewHolder vpvh = mMap.get(mCurPosition);
    ...
    public int getItemCount() {
        if (!DataSafeUtils.isEmpty(mList)) {
            Log.v("tags",mList.size()+"----size");
            return mList.size();
        }
        else
            return 0;
...
//        if (((MainActivity) mContext).isPaused()) {
//            return;
//        }
        ...
        if (vh != null) {
            VideoPlayHomeWrapViewHolder vpvh = ((Vh) vh).mVideoPlayWrapViewHolder;
            if (vpvh != null) {
                vpvh.clickAvatar();
            }
        }
    ...
    class Vh extends RecyclerView.ViewHolder {
        ...
        VideoPlayHomeWrapViewHolder mVideoPlayWrapViewHolder;

        public Vh(VideoPlayHomeWrapViewHolder videoPlayWrapViewHolder) {
    ...
    public void onViewDetachedFromWindow(@NonNull Vh vh) {
        VideoPlayHomeWrapViewHolder vpvh = vh.mVideoPlayWrapViewHolder;
        if (vpvh != null) {
    ...
    private void findCurVideo() {
        ...
        if (position >= 0 && mCurPosition != position) {
            ...
            VideoPlayHomeWrapViewHolder vh = mMap.get(position);
...
//            notifyDataSetChanged();
    ...
    public interface ActionListener {
        void onPageSelected(VideoPlayHomeWrapViewHolder vh, boolean needLoadMore);

        void onPageOutWindow(VideoPlayHomeWrapViewHolder vh);
