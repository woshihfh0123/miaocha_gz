...
public class TraceAdapter extends RecyclerView.Adapter<TraceAdapter.TraceViewHolder> {
    ...
    public void onBindViewHolder(TraceViewHolder holder, int position) {
        ...
        if (type == TYPE_CURR) {
            holder.acceptStationTv.setTextColor(mContext.getResources().getColor(R.color.black));
            holder.dotIv.setImageResource(R.mipmap.dot_red);
//            holder.timeLineView.setBackgroundResource(R.color.green1);
        } else if (type == TYPE_NORMAL) {
            holder.acceptStationTv.setTextColor(mContext.getResources().getColor(R.color.black1));
            holder.dotIv.setImageResource(R.mipmap.dot_black);
//            holder.timeLineView.setBackgroundResource(R.color.line1);
        }
        if (!DataSafeUtils.isEmpty(trace.getRemark())){
            holder.accept_remark_tv.setText(trace.getRemark());
            holder.accept_remark_tv.setVisibility(View.VISIBLE);
        }else{
            holder.accept_remark_tv.setVisibility(View.GONE);
        }
    ...
    public class TraceViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView accept_remark_tv;  //备注
        ...
        public TraceViewHolder(View itemView) {
            ...
            accept_remark_tv = (TextView) itemView.findViewById(R.id.accept_remark_tv);
