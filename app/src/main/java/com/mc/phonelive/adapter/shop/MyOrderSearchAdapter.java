 ...
 * 我的订单
 */
public class MyOrderSearchAdapter extends BaseQuickAdapter<OrderListVo.InfoBean, BaseViewHolder> {
    private Context mContext;
    private String mType;
    BaseQuickAdapter<OrderListVo.InfoBean.GoodsListBean, BaseViewHolder> mFoodListAdapter;

    public MyOrderSearchAdapter(Context context, int layoutResId, @Nullable List<OrderListVo.InfoBean> data, String type) {
...
//        textView.setBackground(BaseCommonUtils.setBackgroundShap(mContext, 5,R.color.FUBColor3, R.color.C1));//边框的颜色和背景的颜色
    ...
    private void sendOperations(OrderListVo.InfoBean.ButtonListBean operationsBean, OrderListVo.InfoBean listBean, String orderId) {
        ...
        if (operationsBean.getId().equals("1")){//删除
            EventBus.getDefault().post(new EventBusModel("setDelData_search", listBean, mType));
        }else if (operationsBean.getId().equals("2")){//取消
            EventBus.getDefault().post(new EventBusModel("setCancleData_search", listBean, mType));
        }else if (operationsBean.getId().equals("3")){//去付款
            EventBus.getDefault().post(new EventBusModel("PayNow_search", listBean, mType));
            ...
            EventBus.getDefault().post(new EventBusModel("ConfirmProduct_search", listBean, mType));
